...
```mermaid
graph TB
    subgraph Manager["数据源管理器"]
        Intel["威胁情报 (Intel)"]
        Geo["地域封禁 (Geo)"]
        Manual["手动规则 (Manual)"]
        Cron["Cron 调度器<br/>定时更新 / 手动触发"]
        IPSum["IPSum / Spamhaus"]
...
flowchart TB
    subgraph First["首次启动"]
        A1[下载数据] --> B1[写入 eBPF Map]
        B1 --> C1[持久化到本地]
    end

    subgraph Later["下次启动"]
        A2[加载本地缓存] --> B2[写入 eBPF Map]
        B2 --> C2[可选: 后台更新]
    end
...
```
source_mask = 0x01 | 0x08 = 0x09 (二进制 00001001)
```

某 IP 同时被手动规则、WAF 和 FailGuard 封禁：

```
source_mask = 0x04 | 0x08 | 0x100 = 0x10C (二进制 000100001100)
```
